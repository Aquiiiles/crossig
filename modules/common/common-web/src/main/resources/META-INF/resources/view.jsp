...
<div class="container-fluid container-fluid-max-xl container-view">
	...
	<aui:form action="${clearCacheURL}" method="post" name="clearCacheFm" onSubmit='<%= "event.preventDefault(); " + renderResponse.getNamespace() + "clearCache('" + renderResponse.getNamespace() + "clearCacheFm');" %>'>
		...
		<div class="row">
			<div class="col-md-2">
				<button class="btn btn-primary gsdc-btn-action" onClick="<%= renderResponse.getNamespace() + "setCacheName('" + ClearCacheMVCActionConstants.CLEAR_CACHE_ALL + "');" %>" type="submit">
					<span class="gsdc-btn-text"><liferay-ui:message key="clear-all-cache" /></span>
				</button>
			</div>

			<div class="col-md-2">
				<aui:input id="cityName" label="city-name" name="cityName" type="text" />

				<button class="btn btn-primary gsdc-btn-action" onClick="<%= renderResponse.getNamespace() + "setCacheName('" + ClearCacheMVCActionConstants.CLEAR_CACHE_CITY + "');" %>" type="submit">
					<span class="gsdc-btn-text"><liferay-ui:message key="clear-cities-cache" /></span>
				</button>
			</div>

			<div class="col-md-2">
				<button class="btn btn-primary gsdc-btn-action" onClick="<%= renderResponse.getNamespace() + "setCacheName('" + ClearCacheMVCActionConstants.CLEAR_CACHE_STREET + "');" %>" type="submit">
