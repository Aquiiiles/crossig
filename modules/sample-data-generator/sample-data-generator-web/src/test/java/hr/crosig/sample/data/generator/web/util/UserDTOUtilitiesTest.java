...
public class UserDTOUtilitiesTest {
	...
	@Test
	public void getUserDTOFromMap_invalidAttributeRequest()
		...
		Method getUserDTOFromMapMethod = Whitebox.getMethod(
			UserDTOUtilities.class, "_getUserDTOFromMap", LinkedTreeMap.class);

		UserDTO userDTO = (UserDTO)getUserDTOFromMapMethod.invoke(
			null, request);
	...
	@Test
	public void getUserDTOFromMap_validRequest()
		throws IllegalAccessException, InvocationTargetException {
		...
		String resultFirstName = userDTO.getUserAttributeValue(
	...
	public void getUserDTOListFromJSONString_invalidRequest() {
		...
		String invalidJSON =
			"$$$$$[" + "{" + UserAttributeEnum.FIRST_NAME + ":" + firstName1 +
				", " + UserAttributeEnum.LAST_NAME + ":'Agent', " +
					UserAttributeEnum.EMAIL_ADDRESS +
						":'john.agent@mailinator.com'," +
							UserAttributeEnum.PASSWORD + ":" +
								CreateUserMVCActionConstants.
																					...
																					":'Manager', " +
																						UserAttributeEnum.EMAIL_ADDRESS +
																							":'jack.manager@mailinator.com'," +
																								UserAttributeEnum.PASSWORD +
																									":" +
																										CreateUserMVCActionConstants.DEFAULT_USER_PASSWORD +
																											", " +
																												UserAttributeEnum.USER_GROUP_NAME +
																													":" +
																														CreateUserMVCActionConstants.USER_GROUP_MANAGER +
																															"}" +
																																"]";

		List<UserDTO> result = UserDTOUtilities.getUserDTOListFromJSONString(
			invalidJSON);
	...
	public void getUserDTOListFromJSONString_multipleUsers() {
		...
		long firstName1Count = result.stream(
		).filter(
			userDTO -> userDTO.getUserAttributeValue(
				UserAttributeEnum.FIRST_NAME
			).equals(
				firstName1
			)
		).count();
		long firstName2Count = result.stream(
		).filter(
			userDTO -> userDTO.getUserAttributeValue(
				UserAttributeEnum.FIRST_NAME
			).equals(
